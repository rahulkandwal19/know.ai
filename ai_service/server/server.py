...
# API Endpoint : askKnowAI - used to get answer of clients prompt form default Know AI model
@app.websocket("/askKnowAI")
async def ask_knowAI(websocket: WebSocket):
    client_id = await manager.connect(websocket) 
    ...
    try:
        while True:
            try:
                ...
                context = '''
                            You are an assistant. Your work is to assist user in knowledge retrival task.
                            Strictly follow instructions. Generate answer strictly in markdown without any syntax error.
                          '''
            ...
            try:
                async for chunk in generate_knowAI_tiny(context,question):
